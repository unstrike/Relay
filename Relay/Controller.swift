...
class Controller {
  ...
  private func openURL(_ action: Action) {
    let label = action.label ?? String(action.value.prefix(100))
    guard let url = URL(string: action.value) else {
      showAlert(
        title: "Invalid URL", message: "Failed to parse URL for action: \(label)")
      return
    }
    ...
    guard let scheme = url.scheme else {
      showAlert(
        title: "Invalid URL",
        message:
          "URL is missing protocol (e.g. https://, raycast://) for action: \(label)"
      )
      return
    }
