...
class UserConfig: ObservableObject {
  ...
  private var _lastReadChecksum: String?
  private let checksumQueue = DispatchQueue(label: "UserConfig.checksum")
  private var lastReadChecksum: String? {
    get { checksumQueue.sync { _lastReadChecksum } }
    set { checksumQueue.sync { _lastReadChecksum = newValue } }
  }
  ...
  var path: String {
    let base = (Defaults[.configDir] as NSString).expandingTildeInPath
    let resolved = URL(fileURLWithPath: base).resolvingSymlinksInPath().path
    guard resolved.hasPrefix(NSHomeDirectory()) else {
      return (Self.defaultDirectory() as NSString).appendingPathComponent(fileName)
    }
    return (resolved as NSString).appendingPathComponent(fileName)
  }
  ...
  func exportConfig(to url: URL) {
    do {
      try fileManager.copyItem(at: self.url, to: url)
    } catch {
      alertHandler.showAlert(style: .critical, message: "Export failed: \(error)")
    }
  }
  ...
  func importConfig(from url: URL) {
    do {
      ...
      if maxNestingDepth(of: decoded) > 20 {
        alertHandler.showAlert(
          style: .critical,
          message: "Import failed: config exceeds maximum nesting depth of 20.")
        return
      }
      let errors = ConfigValidator.validate(group: decoded)
      if !errors.isEmpty {
        let messages = errors.map { $0.message }.joined(separator: "\n")
        alertHandler.showAlert(
          style: .critical, message: "Import failed: config has validation errors:\n\(messages)")
        return
      }
      try writeFile(data: data)
      reloadFromFile()
    } catch {
      alertHandler.showAlert(style: .critical, message: "Import failed: \(error)")
    }
  }

  private func maxNestingDepth(of group: Group, currentDepth: Int = 0) -> Int {
    var maxDepth = currentDepth
    for item in group.actions {
      if case .group(let subgroup) = item {
        let depth = maxNestingDepth(of: subgroup, currentDepth: currentDepth + 1)
  ...
  func resetToDefault() {
    do {
      ...
      alertHandler.showAlert(style: .critical, message: "Reset failed: \(error)")
    }
  }
